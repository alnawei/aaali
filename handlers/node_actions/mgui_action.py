...
async def show_mgui_panel(call: CallbackQuery):
    ...
    text = (
        ...
        f"💡 <b>运维功能指南</b>：\n"
        f"• <b>一键部署</b>：自动配置面板，并<b>自动绑定当前主控 Bot</b>以供单机使用。\n"
        f"• <b>专属绑定</b>：若是多机集群，请点击下方「🤖 绑定专属监控 Bot」配置独立密钥防冲突。\n"
        f"• <b>访问凭证</b>：点击最上方「🔍 实时探测」，即可获取最新端口与密码。\n\n"
...
async def execute_mgui_command(call: CallbackQuery, state: FSMContext):
    ...
    # 🛑 拦截点：如果是绑定专属 Bot，不直接执行命令，而是弹出对话窗进入状态机
    if action == "bind_bot":
        ...
        await call.message.answer(
            ...
            f"由于多个节点使用同一 Token 会产生冲突掉线，建议为该节点设置独立的 Bot。\n"
            f"👉 请回复该节点的 <b>Bot Token</b>：\n\n"
            f"<i>(如不担心冲突强制使用主控配置，请回复 0；发送 /cancel 取消)</i>",
            parse_mode="HTML"
        )
...
"""
    ...
    try:
        ...
        request = ecs_models.RunCommandRequest(
            ...
            instance_id=[instance_id], name=f"MG_Bot_UI_{action}", timeout=180
        )
        ...
        if action == "check":
            ...
            await call.message.edit_text(
                ...
                f"━━━━━━━━━━━━━━━━━━\n"
                f"💡 <i>提示：复制上方账号密码，在浏览器访问 http://实例公网IP:端口 即可进入 Web 后台。</i>",
...
async def mgui_bind_admin(message: Message, state: FSMContext):
    ...
    wait_msg = await message.answer("⏳ 正在向远程服务器底层注入 Bot 专属配置并重启守护进程...")
...
try:
    ...
    try:
        ...
        request = ecs_models.RunCommandRequest(
            region_id=region_id, type='RunShellScript', command_content=encoded_script,
            instance_id=[instance_id], name=f"MG_Bot_UI_bind_custom", timeout=60
        )
        response = await asyncio.to_thread(client.run_command, request)
        invoke_id = response.body.invoke_id
        ...
        if "BIND_SUCCESS" in real_output:
            await wait_msg.edit_text(
                ...
                f"✅ 配置已写入数据库并重启生效，该节点告警已隔离！",
                reply_markup=keyboard, parse_mode="HTML"
            )
        else:
            await wait_msg.edit_text(f"⚠️ 绑定指令已下发，请点击实时探测验证状态。\n回显：{real_output[:50]}", reply_markup=keyboard)
    except Exception as e:
